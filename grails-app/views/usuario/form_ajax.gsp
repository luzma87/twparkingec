        ...
        <g:form class="form-horizontal" name="frmUsuario" id="${usuarioInstance?.id}"
            ...
            <div class="row">
                <div class="col-md-12 form-group ${hasErrors(bean: usuarioInstance, field: 'autos', 'error')} ">
                    <label for="autos" class="col-md-4 control-label">
...
<g:each in="${usuarioInstance?.autos?}" var="a">
    <li><g:link controller="auto" action="show" id="${a.id}">${a?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="auto" action="create" params="['usuario.id': usuarioInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'auto.label', default: 'Auto')])}</g:link>
</li>
</ul>
